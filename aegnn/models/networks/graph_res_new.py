...
class GraphResNew(torch.nn.Module):
    ...
    def __init__(self, dataset, input_shape: torch.Tensor, num_outputs: int, pooling_size=(16, 12),
        ...
        super(GraphResNew, self).__init__()
        ...
        self.fc1 = Linear(pooling_outputs * 16, out_features=pooling_outputs * 16, bias=bias) #修改适配融合的部分
        self.fc2 = Linear(pooling_outputs * 32, out_features=num_outputs, bias=bias)
        # 点输入网络，用来融合两个图网络和待识别的点
        self.point_net = nn.Sequential(
            nn.Linear(3,pooling_outputs),
            nn.ELU(),
            nn.Linear(pooling_outputs,pooling_outputs*16),
            nn.ELU()
        )

    def forward(self, data: torch_geometric.data.Batch) -> torch.Tensor:
        point = data.pos[-1][:] #取出最后一个事件点
        ...
        x = elu(self.fc1(x))
        #训练点的网络+融合
        point = self.point_net(point)
        point = point.repeat(x.shape[0],1)
